...
Citizen.CreateThread(function()
    while true do
        ...
        local distance = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), -245.8976, -916.2994, 32.2158, true)
        if distance <= 5 then
            sleep = 0
            DrawMarker(23, -259.325, -970.129, 30.220,
            ...
            if distance <= 3.2 then
                ...
                if not trabalhando then
                    DrawText3D(-245.8976, -916.2994, 32.2158 + 1.5, Config.Locales["startjob"])
                    if IsControlJustPressed(0, 38) then
                        sleep = 5
                            trabalhando = true
                    ...
                    DrawText3D(-264.156, -991.380, 30.553 + 1.5, Config.Locales["endjob"])
                    if IsControlJustPressed(0, 38) then
                        sleep = 5
                            trabalhando = false
...
function Perto()
    if Config.framework == 'esx' then 
        ...
        if (name == "Rumpo") and distance < 2.5 then
...
function spawnCarJob(car)
    ...
    if Config.framework == 'esx' then 
        ESX.Game.SpawnVehicle(Config.Car,vector3(-262.428, -991.545, 30.364), 79.89, function(veh)
            vehicle = veh
    ...
    elseif Config.framework == 'qbcore' then 
        ...
        end, vector3(-238.7174, -918.9733, 32.2158), true)   
    end
end
